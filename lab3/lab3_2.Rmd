...
```{r}
names(fish) # important because gives the column names, will use a lot
```

`head()` prints the first n rows of the data frame.
```{r}
head(fish) # first few rows
```

`tail()` prinst the last n rows of the data frame.
```{r}
tail(fish) # last few rows
```
...
```{r}
table(fish$lakeid) # very helpful, fast count the number in a variable
```

## Select
Select is a way of sorting data frames by pulling out variables (columns) of interest.
```{r}
select(fish, lakeid, length) # only columns ( chart name, the columns you want to look at)
```
...
```{r}
filter(fish, length<=100) # horizontallay (row) (chart name, the rows you want to look at)
```
