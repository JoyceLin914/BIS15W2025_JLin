...
```{r}
hsprings <- tibble(temp, scientist=name, spring) #order of list = order of columns to appear first
hsprings
```
...
```{r}
write.csv(hsprings, "hsprings.csv", row.names = FALSE) #prevents R from adding a new column with numbers
```
...
```
```{r}
summary(hot_springs) #be careful with the math, is not always accurate
```
...
```{r}
hot_springs$scientist <- as.factor(hot_springs$scientist) # factor = character that has limited observations to be repeated
```
