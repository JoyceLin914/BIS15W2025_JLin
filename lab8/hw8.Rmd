...
there are 0 NAs in amniota but I don't think it makes sense because I know that the NAs are represented as -999 on the data.
```{r}
amniota %>% 
  miss_var_summary() %>% 
  arrange(-n_miss)
...
The total NAs for each variable is shown in the table, and the data makes sense.
```{r}
amphibio %>% 
  miss_var_summary() %>% 
  arrange(-n_miss)
