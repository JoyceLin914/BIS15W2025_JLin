...
```{r}
life_history %>% 
  summarize(across(everything(), ~ sum(is.na(.)))) # look how many NA across the data frame
# ~=lambda function
```
...
life_history_no_nas <- life_history_no_nas %>% 
  mutate(max_life=na_if(max_life, 0)) #replace 0 with NA
```
...
life_history_no_nas %>%
  group_by(order) %>%
  miss_var_summary(order=T) # more specific idea of where NA are in the data
```
...
```{r}
common_na_strings #can copy and paste; an example of strings; people might represents NA in different ways, so this is a good way to scan the data and find all the nas
```

```{r}
common_na_numbers # numbers often represent NAs 
```
...
cites %>% 
  ...
  group_by(class) %>% 
  miss_var_summary() %>% 
  arrange(-n_miss) #n_miss= number of NAs
```
...
#only use this once if you are sure that NAs are represent the way that you want 
```
