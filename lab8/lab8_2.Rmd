...
R wants long data-> to make plots-> need long data
...
```{r}
heartrate <- read_csv("data/heartrate.csv") #not tidy because abcd are drugs and not correlate to heart rate
heartrate
```
...
because the column name doesn't match the values
...
relig_income %>% 
  pivot_longer(-religion, # not moving religion
               ...
               values_to = "n_household") # values to a new variable "n_household"
```
...
billboard2 <- 
  billboard %>% 
  pivot_longer(wk1:wk76, # a range of columns/ can specify the range of columns you want to move
               names_to = "week",
               values_to = "rank", 
...
billboard3 <- 
  billboard %>% 
  pivot_longer(-c(artist, track, date.entered), #specific columns to stay fixed and move everything else
               names_to = "week",
               values_to = "rank",
               values_drop_na = TRUE
               )
...
the number of days should have its own column and the values should have its own column
...
qpcr_untidy %>% 
  pivot_longer(
    exp1_rep1:exp3_rep3, # pivot these columns
    names_to= c("experiment", "replicate"), 
    names_sep="_", # separate the names
