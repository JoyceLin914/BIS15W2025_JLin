...
```{r}
x %>% summarize(n=n()) # keep the groupings, keep island and species
```
...
```{r}
y %>% summarize(n=n()) # drop all the groups, no island and species
```
...
z <- penguins %>% 
  group_by(species, island) %>% 
  summarize(mean_bill_length_mm=mean(bill_length_mm, na.rm = T), .groups = 'drop_last') #default
```
