...
sharks_2 %>% 
  ...
  summarize(ave_catch=mean(catch, na.rm=T))
```
...
Which fishing area has the highest total catch in a year between 1995 and 2010?
```{r}
fisheries %>% 
  group_by(fao_major_fishing_area, year) %>% 
  filter(between(year, 1995,2010)) %>% 
  summarize(sum_catch=sum(catch, na.rm=T), .groups = "keep") %>% 
  arrange(-sum_catch)
```
