...
## `pivot_longer()`
Recall that we use `pivot_longer()` when our column names actually represent variables. A classic example would be that the column names represent observations of a variable. Observations of variables 
```{r}
...
```{r}
heartrate2 <- read_csv("data/heartrate2.csv") # can't use pivot because it is worse, it is not tidy because 2 pieces of data in the same column
heartrate2
```
...
```{r}
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = -1) #remove last character of the patient string
```
