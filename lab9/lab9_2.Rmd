...
```{r}
glimpse(iris) # group project need continuous numerical data and factors
```
...
ggplot(data=iris, #data
       mapping = aes(x=Species, y=Petal.Length)) + 
  geom_point() # x is categorical and y is continuous so the plot looks weird
```
...
homerange %>% 
  ...
  geom_bar() # use this for categorical data
```
...
homerange %>% 
  ...
  select(common_name, log10_mass) %>% 
  ggplot(aes(x=common_name, y=log10_mass))+
  geom_col() + # specifying x and y so use col
  coord_flip()
```
