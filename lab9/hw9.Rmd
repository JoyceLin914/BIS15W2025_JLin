...
_It is pretty tidy because each row is an observation and each variable is a column, but if we have to say it, the date column is not as tidy as the other columns because each cell of the column consists of day, month, and year, so maybe separate them will make the data fit the defintions of tidy better. It is in long format._
```{r}
sydneybeaches
```
...
For little bay beach, which year was polluted the most?
```{r}
sydneybeaches_long %>% 
