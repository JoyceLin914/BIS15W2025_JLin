...
```{r}
glimpse(fish)# important because change class of a variable
```

Now we can pull out any state of interest.  
```{r}
filter(fish, lakeid == "LSG")
```
...
```{r}
filter(fish, lakeid =="AL" | length> 350) # | equals to "or"
```
...
select the data of interest -> filter the specific data from the data of interest
```{r}
...
```{r}
mammals2 <- filter(mammals, family == "Felidae") # or can use & gestation <3 and delete mammals3
mammals3 <- filter(mammals2, gestation < 3)
mammals4 <- select(mammals3, "genus", "family", "gestation")
filter(mammals4, gestation!=-999)
```
