...
```{r}
filter(fish, lakeid == "AL") # reading sideways, reading rows
# has 2 equals because it is able to go both directions. AL equals to lakeid, and lakeid equals to AL
```
...
```{r}
filter(fish, length %in% c(167, 175)) # length is a vector in a data frame
```



```{r}
filter(fish, length==167) # can only get lengths that exactly equals to 167 to show up
```
