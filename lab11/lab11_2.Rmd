...
```{r}
p +geom_point(size=.5) #controlling size of points
```
...
```{r}
p+geom_point(aes(shape=thermoregulation, color=thermoregulation), size=1.25) # different shapes and colors for dots
```
...
```{r}
homerange %>% 
  ggplot(aes(x=taxon, fill=trophic.guild))+ # stacked bar plot
  geom_bar()+
  coord_flip()+
  labs(title="Observation by Taxon",
       x="Taxonomic Group",
...
```{r}
homerange %>% 
  ggplot(aes(x=class, y=log10.mass, group=taxon))+ #mini plots for each class
  geom_boxplot()
```
