...
`geom_col` specify an x and y so need count
```{r}
life_history %>% 
  count(order) %>% 
...
`geom_bar` no need to count, just straight to plot
```{r}
life_history %>% 
  ggplot(aes(x=order))+
  geom_bar()+
...
life_history %>% 
  group_by(order) %>% 
  summarize(ave_order=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=order, y=ave_order))+
  geom_col()+
  coord_flip()
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(ave_mass=mean(mass, na.rm=T))
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(ave_order=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order, ave_order), y=ave_order))+ # reorder the variable order by the mean of mass (like rearrange but in ggplot)
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
