...
mammals %>% 
  ...
  mutate(gestation_years=gestation/12) %>% #mutate = adding a new variable
...
```{r}
mammals %>%
  mutate(across(-1, ~ tolower(.))) #apply tolower to all columns (except vraiables in order) except the first 

# across = go across the entire data frame
#lambda or anoymous function
# "." = "do this to every cell"
```
...
```{r}
summary(msleep24) # make sure no -999 values
```
