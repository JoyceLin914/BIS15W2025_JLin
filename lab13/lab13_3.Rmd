...
```{r}
map1 <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom=7) #if zoom is too big, will take a long time to load; no need to use stamen_terrain because there are some other maps; if too big, it will cause some problems
```
...
```{r}
ggmap(map1) + 
  geom_point(data = spiders, aes(longitude, latitude), size=0.4) + # data= meaning not using pipes but layers
...
```{r}
ggmap(map2) + 
  geom_point(data = sharks_dups, aes(x=longitude, y=latitude), size=0.8) + # data= meaning not using pipes but layers
  labs(x= "Longitude", y= "Latitude", title="Shark Attack in all years")
```
...
bbox3 <- make_bbox(lon3, lat3, f=0.3)
```
...
```{r}
ggmap(map3)+
  geom_point(data = sharks_fatal, aes(x=longitude, y=latitude), size=0.8) + # data= meaning not using pipes but layers
  labs(x= "Longitude", y= "Latitude", title="Shark Fatal Attack")
```
