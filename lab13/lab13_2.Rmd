...
```{r}
inner_join(table_A, table_B, by="customer_ID") #only joining stuff that matches
```

`left_join`
Where did customer 4 go? Notice the missing data for customer 3.
```{r}
left_join(table_A, table_B, by="customer_ID") #join the right table to left table by only the customer ID on left table
```

`right_join`
Where did customer 3 go? Notice the missing data for customer 4.
```{r}
right_join(table_A, table_B, by="customer_ID")#join the left table to right table by only the customer ID on right table
```

`full_join`
```{r}
full_join(table_A, table_B, by="customer_ID") # join both tables together by customer ID
```

`anti_join`
Provides the rows in the first table for which there are not matching values in the second table.
```{r}
anti_join(table_A, table_B, by="customer_ID") # provide the rows from left table which are not matching values in the right table
...
_doesn't matter since both tables have the same number of rows_
```{r}
spiders_with_locs <- 
