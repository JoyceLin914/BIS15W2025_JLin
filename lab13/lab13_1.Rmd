...
p + theme_linedraw()+
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL, # no name for x-axis because the name is obvious
       y= "n",
       fill= "Trophic Guild")
```
...
```{r}
q+theme_classic() # theme can allow us to make nice plots
```
...
```{r}
p+theme_linedraw()+
  theme(legend.position = "top", #adjust the position of the legend
...
```{r}
?RColorBrewer #design for mapping, especially political maps
```
...
+`scale_colour_brewer()` is for points  for scatter plot
+`scale_fill_brewer()` is for fills  for bar plot

Here I chose the `Set2` palette. Take a moment and experiment with other options.
```{r}
q+scale_colour_brewer(palette = "9-class OrRd")+ #used for geom_point
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Mass vs. Preymass",
       x = "Mass",
       y= "Preymass")
```
...
```{r}
p+scale_fill_brewer(palette = "9-class OrRd")+
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
...
homerange %>% 
  ...
  geom_boxplot()+
  facet_wrap(~trophic.guild, ncol=2, labeller=label_both)+ #wrap by trophic guild into 2 columns
  theme(axis.text.x = element_text(angle = 60, hjust=1),
...
homerange %>% 
  ...
  geom_boxplot()+
  facet_grid(trophic.guild~.)+ #split the carnivore and herbivores into 2 parts
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
...
## Practice
1. Use facet_wrap to compare the distribution of `log10.mass` by `thermoregulation`. #when midterm asks "distribution", use facet_wrap
```{r}
homerange %>% 
  ggplot(aes(log10.mass))+
  geom_density(fill="steelblue", alpha=0.6)+
  facet_wrap(~thermoregulation) # it only has 2 categories, so can also use facet_grid
```
...
homerange %>% 
  ggplot(aes(log10.mass))+
  geom_density(fill="steelblue", alpha=0.6)+
  facet_grid(locomotion~.) #rows by columns; locomotion at rows
```
...
homerange %>% 
  ggplot(aes(log10.mass))+
  geom_density(fill="steelblue", alpha=0.6)+
  facet_grid(.~locomotion) #locomotion at columns
```
