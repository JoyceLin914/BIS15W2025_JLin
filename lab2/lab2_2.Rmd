...
## Load the tidyverse
```{r}
library("tidyverse") # collection of pack to work efficiently
# need to load this for things to work
```
...
A numeric vector.
```{r}
my_vector <- c(10, 20, 30) # vector is not a list but more like a sequence
```
...
```{r}
Sex <- c("male", "female", "male") #c tells you that it is a vector
...
```{r}
hbirds <- data.frame(Sex, Length, Weight) # data.frame is one way to make a data frame
```
...
hbirds # class dbl = double precision floating point
```
...
```{r}
names(hbirds) # give you the column names
```


```{r}
# weight and length need to be capitalized in labels
```
...
```{r}
write.csv(hbirds, "hbirds_data.csv", row.names = FALSE) # will save this as an independent csv file
```
