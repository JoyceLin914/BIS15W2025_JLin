...
```{r}
z=10 #do not use = because unprofessional
```
...
```
```{r}
# option commend i = new chunk
# control alt i for pc
```
...
```{r}
#control1 # comment out stuff like python
```
...
```{r}
mean(c(2, 8, 2)) # calculate the mean of 2 8 2
```
...
```{r}
mean(venum_GT, chiron, veyron) # can push tab to get autocomplete
```
...
# integer has to be a whole number
...
NA means there is no value for that data.
NA tells us that they can't calculate the mean number if it appears. It is not applicable because there is a data that is not a numeric number, so we can't calculate the number


4. Recalculate the mean using the following code chunk. Why is the useful?  

```{r}
mean(new_vector, na.rm=T) # na.rm will remove an NA
```
