...
_name, genus, vore, order, conservation, sleep_total, sleep_rem, sleep_cycle, awake, brainwt, bodywt_
```{r}
names(msleep)
```
...
server <- function(input, output, session) {
  session$onSessionEnded(stopApp)
    output$plot <- renderPlot({
      ...
      ggplot(data=msleep,
             aes_string(x="vore", y=input$x, fill="vore"))+
        geom_boxplot()+
...
msleep %>% 
  ...
  labs(title = "Total Sleep Time Distribution for Each Vore Diet", 
       x= "Total Sleep Time",
       y=NULL)
```
